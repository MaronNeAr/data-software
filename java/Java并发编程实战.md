...
- **CAS 操作（无锁并发的灵魂）**：JUC 里的 `Atomic` 系列（如 `AtomicInteger`）核心就是 `compareAndSwapInt`。它通过 CPU 指令保证了“比较并交换”的原子性，实现了无锁化的线程安全。
- **内存操作（堆外内存管理）**：提供 `allocateMemory`、`freeMemory`、`putInt` 等方法，允许直接在 OS 内存中开辟空间，不受 JVM GC 管控。
- **线程调度（锁的底层实现）**：提供 `park` 和 `unpark` 方法；当你调用 `reentrantLock.lock()` 失败进入等待时，底层调用的就是 `Unsafe.park()`。
- **对象偏移量（直接读写字段）**：可以通过 `objectFieldOffset` 获取某个变量在内存中的相对位置，然后直接修改。
- **内存屏障（可见性保证）**：提供 `loadFence`、`storeFence`、`fullFence`。
...
- **volatile 关键字**：在写 `volatile` 变量后插入 **StoreLoad 屏障**，确保前面的写对后面可见。
- **final 关键字**：确保构造函数内的初始化在对象引用可见前完成。
- **Happens-Before 原则**：JMM 定义的一套规则（如：锁的释放必然早于后续锁的获取），规定了哪些场景下禁止重排序。

#### 内存屏障

###### LoadLoad 屏障

- **规则**：`Load1; LoadLoad; Load2` 
- **含义**：确保 `Load1` 加载的数据先于 `Load2` 及其后所有加载指令被读取。
- **举例**：在多线程环境下，你先读取一个“状态位”，只有当状态位符合预期时，才去读取真实的数据。

###### StoreStore 屏障

- **规则**：`Store1; StoreStore; Store2` **含义**：确保 `Store1` 的数据在 `Store2` 及后续写入操作执行前，已经刷入主内存，并对其他处理器可见。
- **举例：构造函数中的 `final` 字段**。
- **效果**：保证当其他线程看到 `Task` 对象引用时，`id` 字段一定已经初始化完成了，不会看到 `id=0` 的情况。

###### LoadStore 屏障

- **规则**：`Load1; LoadStore; Store2` 
- **含义**：确保 `Load1` 数据加载先于 `Store2` 及后续写入指令刷新到主内存。
- **举例**：在一个循环中读取数据并根据逻辑写回结果。它可以防止写操作“抢跑”到读操作之前，避免基于错误前提的写入。

###### StoreLoad 屏障（全能屏障）

- **规则**：`Store1; StoreLoad; Load2` 
- **含义**：确保 `Store1` 的写入对其他处理器可见，先于 `Load2` 及后续全部指令执行。
- **效果**：当一个线程修改了 `volatile` 变量，该屏障确保后续任何读操作都能看到这个最新的值。它同时具备了上述三种屏障的功能。
...
| **状态** | **名称**              | **含义**                                                     |
| -------- | --------------------- | ------------------------------------------------------------ |
| **M**    | **Modified (已修改)** | 该行数据只存在于当前 Cache 中，且已被修改，与主存不一致。**当前核负有写回主存的责任。** |
| **E**    | **Exclusive (独占)**  | 该行数据只存在于当前 Cache 中，但与主存一致。随时可以变更为 M 状态。 |
| **S**    | **Shared (共享)**     | 该行数据在多个 CPU 的 Cache 中都存在，且与主存一致。读取时不需要通知其他核。 |
| **I**    | **Invalid (无效)**    | 该缓存行数据失效，不能使用。必须重新从主存或其它 Cache 中读取。 |

###### 状态转换的“潜规则”

- **写操作（Store）**：如果一个核想写数据，它必须确保该 Cache Line 处于 **M** 或 **E** 状态。如果当前是 **S**，它必须发送一个 **Invalidate（失效）消息** 给其他所有拥有该行数据的核，等收到它们的 **ACK（确认）** 后，才能将状态改为 **M** 并写入。
- **读操作（Load）**：如果一个核想读数据且当前是 **I** 状态，它会发送 **Read 消息**。其他核如果拥有该数据的最新副本（M 状态），必须先将数据刷回主存或直接转发给请求核，然后大家一起变为 **S** 状态。

###### CPU缓冲机构

- **Store Buffer（写缓冲区）**
  - **副作用**：此时 MESI 还没更新，其他核读到的还是旧值。这就是**指令重排序（内存系统重排）**的根源。
  - **解决**：`volatile` 产生的 **StoreLoad 屏障**会强制 CPU 等待 Store Buffer 清空，确保 MESI 状态更新完成。
- **Invalidate Queue（失效队列）**
  - **副作用**：这个核在短时间内还是能读到旧的（本该失效的）数据。
  - **解决**：内存屏障会强制处理失效队列，确保读到的是最新状态。

###### 伪共享

- MESI 的状态管理是以 **Cache Line（64 字节）** 为单位的。

- 如果变量 `A`（被 Core 1 修改）和变量 `B`（被 Core 2 修改）恰好落在同一个缓存行：

  - Core 1 修改 A，导致 Core 2 的缓存行失效。

  - Core 2 修改 B，导致 Core 1 的缓存行失效。

- **结果**：两个 CPU 核在不断地互踢对方的缓存，导致性能雪崩。这就是**伪共享**。

- **对策**：通过字节填充（Padding）让 A 和 B 位于不同的缓存行。
...
- **资源昂贵**：一个平台线程默认占用 1MB 栈空间，且创建、销毁需要系统调用。
- **并发瓶颈**：受限于 OS 线程数（通常几千个就会导致系统 OOM 或上下文切换过载）。
- **阻塞模型**：在 I/O 密集型业务中，线程大部分时间在等待响应，导致 CPU 利用率极低。

###### 调度模型：M:N 映射机制

- **M（虚拟线程）**：你可以创建数百万个，它们存储在 JVM 堆内存中。
- **N（载体线程/内核线程）**：通常等于 CPU 的核心数。

###### 挂起与恢复的原理（Continuation）

- **Mount（挂载）**：当虚拟线程准备执行时，调度器将其挂载到一个载体线程上。
- **Yield（礼让/挂起）**：当虚拟线程遇到阻塞 I/O（如 `Socket.read` 或 `ReentrantLock`）时，它会触发 `Continuation.yield()`。
  - **关键动作**：JVM 会将当前虚拟线程的**栈信息（Stack Frames）**从载体线程拷贝到**堆（Heap）**中保存。
  - **释放**：载体线程被释放，去处理其他虚拟线程。
- **Unmount（恢复）**：I/O 准备就绪后，调度器从堆中将栈信息拷贝回载体线程，继续执行。

###### 虚拟线程的“副作用”：固定 (Pinning) 风险

- **触发场景**：执行 `synchronized` 代码块或方法时；调用本地方法（Native Method，如 JNI）时。

- **后果**：如果所有载体线程都被固定，整个应用的吞吐量会急剧下降，甚至导致死锁。

- **对策**：将 `synchronized` 替换为 `ReentrantLock`，因为 JUC 包下的锁已经针对虚拟线程进行了优化，能够触发 `yield` 释放载体线程。

###### 关键特性与设计变化

| **特性**                                | **变化与影响**                                               |
| --------------------------------------- | ------------------------------------------------------------ |
| **栈内存**                              | 动态调整。初始极小（几百字节），根据需要增长，且存在堆中。   |
| **线程本地变量 (ThreadLocal)**          | 依然可用，但由于虚拟线程数量极多，使用 `ThreadLocal` 容易导致 OOM，建议使用 `ScopedValue`。 |
| **结构化并发 (Structured Concurrency)** | 与虚拟线程配合使用，确保父子线程生命周期一致，防止线程泄漏。 |
| **无须池化**                            | **不要池化虚拟线程！** 它的创建成本极低，用完即弃。就像 `new Object()` 一样。 |
...
- **State（同步状态）**：一个 `volatile int` 类型的变量，代表资源的状态。
  - 在 `ReentrantLock` 中，`state=0` 表示未锁定，`state>0` 表示已锁定（数值代表重入次数）。
  - 在 `CountDownLatch` 中，`state` 代表剩余需要倒计时的数量。
- **CLH 变体队列**：双向 FIFO 队列，当线程获取资源失败时，会被封装成一个 `Node` 节点，挂载到队列尾部并进入自旋或阻塞状态。
- **CAS（原子更新）**：利用 `compareAndSetState` 来竞争修改 `state` 的值，确保状态转换的原子性。

###### 独占模式 vs. 共享模式

- **独占模式 (Exclusive)**：每次只有一个线程能持有资源。
  - 典型的实现是 `ReentrantLock`。
  - **面试细节**：`tryAcquire` 和 `tryRelease` 由子类实现。
- **共享模式 (Shared)**：允许多个线程同时持有资源。
  - 典型的实现是 `Semaphore`（信号量）和 `CountDownLatch`。
  - **面试细节**：`tryAcquireShared` 返回值代表剩余资源数，大于 0 则表示后续线程仍可获取。

###### 关键流程：入队与阻塞 (acquire)

- **tryAcquire**：首先尝试获取锁，如果成功直接返回。
- **addWaiter**：失败后，将当前线程包装成 Node 放入队尾。
- **acquireQueued**：在队列中自旋。如果前驱节点是 Head，会再次尝试抢锁。
- **Park**：如果还是抢不到，调用 `LockSupport.park()` 让线程进入等待状态（从用户态切换到内核态，这是性能损耗的大头）。

#### 锁机制

###### 锁的公平性与非公平性

- **公平锁**：线程在获取锁之前先检查队列。如果队列里有人在排队，自己就乖乖去排队。

- **非公平锁**：线程上来先尝试 CAS 抢锁，抢不到再去排队。

- **性能优势**：非公平锁性能通常更高，因为它减少了线程唤醒和上下文切换的间隙，利用了“刚释放锁时，刚好有新线程 CPU 还在活跃状态”的巧合。

###### 锁的升级与优化

- **无锁 -> 偏向锁 -> 轻量级锁 -> 重量级锁**。
- **锁粗化 (Lock Coarsening)**：把多次连续的加锁操作合并为一次，减少性能消耗。
- **锁消除 (Lock Elimination)**：通过**逃逸分析**，如果发现对象不会被多线程共享，JIT 会直接把锁去掉。
